...
Napi::Value MediaStreamTrack::GetSettings(const Napi::CallbackInfo &info) {
  ...
  if (_track->kind() == _track->kAudioKind) {
    // NOTE: Designated initializers require C++20, but this addon targets
    // C++17 on Windows/MSVC.
    AudioMediaTrackSettings settings;
  ...
  // NOTE: Designated initializers require C++20, but this addon targets C++17
  // on Windows/MSVC.
  VideoMediaTrackSettings settings;
